...
%% @doc Accessor functions for goldrush event terms.
-module(gre).
...
%% @doc Prepend a key-value pair to an event.
-spec append(term(), event()) -> event().
append(KeyVal, {list, List}) ->
    {list, [KeyVal|List]}.

%% @doc Merge two events into one.
...
merge_test() ->
    E1 = gre:make([{a,1}], [list]),
    E2 = gre:make([{b,2}], [list]),
    Merged = gre:merge(E1, E2),
    ?assert(gre:has(a, Merged)),
    ?assert(gre:has(b, Merged)).
