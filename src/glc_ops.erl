%% @doc Built in operators for event filtering.
%%
%% Provides comparison, existence, and composition operators used
%% to construct event filter queries. Extended operator set includes
%% less-than-or-equal, greater-than-or-equal, not-equal, wildcard
%% (field exists), and not-found (field absent).
-module(glc_ops).
...
%% @doc Test that a field exists (any value).
-spec wc(atom()) -> op().
...
%% @doc Test that a field is not found in the event.
-spec nf(atom()) -> op().
...
%%
%% For an input to be considered valid output all filters specified
...
%%
%% For an input to be considered valid output one of the filters specified
...
%% in a `badarg' error. Accepts arity-1 (event only) or arity-2
%% (event + stored context) callback functions.
