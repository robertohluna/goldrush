%% @doc Code generation functions for compiled query modules.
%%
%% Generates, compiles, and loads Erlang modules at runtime from query
%% filter specifications. The generated module contains optimized pattern
%% matching code that evaluates event filters without interpretation overhead.
-module(glc_code).
...
-spec abstract_module(atom(), #module{}, boolean()) -> {ok, forms, list()}.
...
%% @private Generate an abstract dispatch module.
-spec abstract_module_(atom(), #module{}, boolean()) -> [?erl:syntaxTree()].
    ...
    AbstractMod = [
     ...
     ?erl:attribute(
       ...
       [?erl:list([
        ...
        ?erl:arity_qualifier(?erl:atom(handle), ?erl:integer(1))])]),
     %% get(Name) -> Term.
     ?erl:function(
        ?erl:atom(get),
        abstract_get(Data) ++
     ...
     %% runjob(Fun, Event) - timed job execution entry
     ?erl:function(
       ?erl:atom(runjob),
         ...
         [abstract_count(job_input, State),
          ?erl:application(none,
            ?erl:atom(job_), [?erl:variable("Fun"),
                              ?erl:variable("Event")])])]),
     %% handle_(Event) - filter dispatch
     ?erl:function(
        ?erl:atom(handle_),
        [?erl:clause([?erl:variable("Event")], none,
         abstract_filter(Tree, Data, State))]),
     %% job_(Fun, Meta) - execute and route result
     ?erl:function(
        ?erl:atom(job_),
     ...
     %% job_result(Result, Meta) - process timed result
     ?erl:function(
        ?erl:atom(job_result),
        abstract_runjob(Data, State)
        )
    ],
    %% Transform Term -> Key to Key -> Term in params table
...
abstract_query({all, [{with, _Q, _A}|_] = I}) ->
    ...
    [?erl:abstract(Queries)];
abstract_query({ok, {serialized, Other}}) ->
...
%% @private Generate the job result handler clauses.
...
abstract_count(_Counter, #state{statistics=false}, Value) when is_integer(Value) ->
    ?erl:abstract(Value);
abstract_count(_Counter, #state{statistics=false}, _Value) ->
    ?erl:abstract(0);
...
-spec abstract_resetcount(atom() | [atom()]) -> [syntaxTree()].
