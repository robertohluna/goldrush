...
%% @doc Query processing and reduction functions.
-module(glc_lib).
...
%% @private Factor out common filters.
common({all, Conds}) ->
...
default_is_output_test_() ->
    [?_assertEqual(output, glc_lib:onoutput(glc:lt(a, 1))),
     ...
     ?_assertEqual(output, glc_lib:onoutput(glc:neq(a, 1))),
...
matches_test_() ->
    ...
    [?_assertEqual(true, glc_lib:matches(glc:lt(a, 3), Event)),
     ...
     ?_assertEqual(true, glc_lib:matches(glc:neq(a, 3), Event)),
     ...
     ?_assertEqual(false, glc_lib:matches(glc:neq(a, 2), Event)),
